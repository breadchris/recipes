...
export default function DanRandomPage() {
  ...
  const fetchRandomSection = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/admin/dan-random');
      if (!response.ok) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-zinc-100">Dan Random</h1>
        {currentSection && (
          <Link
            href={`/admin/recipe/${currentSection.videoId}`}
